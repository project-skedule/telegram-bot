...
async def register_teacher_handlers():
    ...
    async def teacher_menu_handler(call: CallbackQuery, state: FSMContext):
        ...
        if await state.get_state() == States.teacher_day_of_week.state:
            ...
            logger.info(
                f"{message.chat.id} | {message.chat.username} | Teacher | teacher_menu | back_misc_menu_second_button | None"
            )
        await send_message(
            message,
    ...
    async def teacher_day_of_week_handler(call: CallbackQuery):
        message = call.message
        logger.info(
            f"{message.chat.id} | {message.chat.username} | Teacher | teacher_day_of_week_menu | day_of_week_menu_button | None"
        )
        await send_message(
            message,
    ...
    async def teacher_choose_day_of_week_handler(
        ...
        logger.info(
            f"{message.chat.id} | {message.chat.username} | Teacher | teacher_day_of_week | day_of_week_button | {day_of_week}"
        )
    ...
    async def teacher_misc_menu_first_handler(
        ...
        if await state.get_state() == States.teacher_menu.state:
            ...
            logger.info(
                f"{message.chat.id} | {message.chat.username} | Teacher | teacher_misc_menu_first | arrow_button | None"
            )
    ...
    async def teacher_misc_menu_second_handler(
        ...
        message = call.message
        logger.info(
            f"{message.chat.id} | {message.chat.username} | Teacher | teacher_misc_menu_second | arrow_button | None"
        )
        await send_message(
            message,
            text=Texts.teacher_misc_menu,
    ...
    async def teacher_today_handler(call: CallbackQuery):
        message = call.message
        logger.info(
            f"{message.chat.id} | {message.chat.username} | Teacher | teacher_today | today_button | None"
        )
    ...
    async def teacher_tomorrow_handler(call: CallbackQuery):
        message = call.message
        logger.info(
            f"{message.chat.id} | {message.chat.username} | Teacher | teacher_tomorrow | tomorrow_button | None"
        )
    ...
    async def teacher_week_handler(call: CallbackQuery):
        message = call.message
        logger.info(
            f"{message.chat.id} | {message.chat.username} | Teacher | teacher_week | week_button | None"
        )
    ...
    async def teacher_ring_timetable_handler(call: CallbackQuery):
        message = call.message
        logger.info(
            f"{message.chat.id} | {message.chat.username} | Teacher | teacher_ring_timetable | ring_timetable_button | None"
        )
    ...
    async def teacher_canteen_timetable_handler(call: CallbackQuery):
        message = call.message
        logger.info(
            f"{message.chat.id} | {message.chat.username} | Teacher | teacher_canteen_timetable | canteen_timetable_button | None"
        )
    ...
    async def teacher_contact_devs_handler(call: CallbackQuery):
        message = call.message
        logger.info(
            f"{message.chat.id} | {message.chat.username} | Teacher | teacher_contact_devs | contact_devs_button | None"
        )
    ...
    async def teacher_support_devs_handler(call: CallbackQuery):
        message = call.message
        logger.info(
            f"{message.chat.id} | {message.chat.username} | Teacher | teacher_support_devs | support_devs_button | None"
        )
    ...
    async def teacher_announcements_handler(call: CallbackQuery):
        message = call.message
        logger.info(
            f"{message.chat.id} | {message.chat.username} | Teacher | teacher_announcements | announcements_button | None"
        )
    ...
    async def teacher_free_cabinets_handler(call: CallbackQuery):
        message = call.message
        logger.info(
            f"{message.chat.id} | {message.chat.username} | Teacher | teacher_free_cabinets | free_cabinets_button | None"
        )
